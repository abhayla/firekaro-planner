...
// AppBar, so it carries no "Viewing as <member>" control to drive, REGARDLESS of how many members
// the household ends up with (the express path can create a spouse and kids — cards 6/7 — so
// "no second member" is not the reason). Its result screen embeds the SAME `<FireHero />` the
// dashboard uses, and `/fire-goals/dashboard` IS swept below — so the lensed surface the quick
// path exposes is covered where the control actually exists. (T-378C finding F6: an earlier
// version of this comment also claimed no second member exists, which is false — corrected here.)
