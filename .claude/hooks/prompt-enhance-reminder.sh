...
echo "ALWAYS SHOW THE ENHANCED PROMPT (format A — Abhay 2026-06-09, MANDATORY, the #1 thing he checks): the *Enhanced:* one-liner is NOT enough. On a non-trivial prompt you MUST render a compact block — 'What changed: <fixes/additions>' + 'Final prompt executed: <the exact strengthened prompt>'. On a trivial/continuation prompt (yes/go-ahead) render the one-liner '*Enhanced: no change — ran your input as-is*'. Skipping the block is a defect (logged to .enhance-misses.log). SSOT: prompt-auto-enhance-rule.md 'MANDATORY OUTPUT'."
