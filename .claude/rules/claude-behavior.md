...
1. **Plan Before Coding**: For any non-trivial change (3+ files/steps, architecture, new feature, schema, refactor, or financial math), produce a **visible plan** — plan mode, a goal contract, or an inline plan block — BEFORE the first code edit. The plan MUST show WHY-this-approach (not just what), the concrete file list, and verification steps; re-plan immediately if it goes sideways. Skip only trivial/mechanical edits (typo, one-line fix, rename). **SSOT + full trigger/exemption list: `.claude/rules/plan-before-coding.md`.**
...
## Output Plausibility (substance, not just shape)

31. **Verify the output is PLAUSIBLE, not just that it renders/passes (MANDATORY)**: a user-facing value — above all a financial number — can render cleanly, type-check, and pass the unit tests while being **domain-absurd** (a 30-year-old retiring at 81). Before declaring any change to a user-facing output done, MUST apply a **semantic sanity check** ("would the persona/a domain expert flinch at this number?") and STOP + root-cause if it's off — never accept mechanical-green. MUST verify on the **DEFAULT product path/lens** a user actually sees (not a convenient one — that divergence hid bug #22); MUST add a sane-bounds assertion to `src/lib/headline-plausibility.spec.ts` for any new flagship output; and for financial-math changes MUST have the **FinTech Domain Analyst** validate the **end-to-end headline** against persona-sane bounds, not just the engine internals. Shape locks ("matches current computation") are NOT correctness proofs — pair each with a substance assertion (sane bounds / a numerator-denominator-same-set coherence invariant / agreement with an independent path). Full rule: `.claude/rules/output-plausibility-verification.md`. Extends rules 24/25/26 (which verify render/persist/cross-page = shape) with the missing substance axis.

## Interactive Functionality (exercise it — don't just render it)

32. **Verify interactive FUNCTIONALITY, not only that the screen renders (MANDATORY)**: every UI verification — at EVERY layer (the rules 24/26 dev-time checks, E2E specs, AND production smoke) — MUST exercise the screen's interactive functionality: clicks, tab switches, FY/period selectors, form fill + submit, dialog open/save/cancel, filters, expand/collapse, and the screen's primary action(s) — not merely confirm it renders / has no console errors / looks right. **"It renders" is SHAPE; "it functions" is SUBSTANCE** — a screen can render perfectly with every control dead (the exact gap behind the 2026-06-04 "verified" overclaim). For each screen, pick the interactions that matter and confirm each *responds* (state changes, data updates, dialog opens, FIRE figure recomputes, no NEW console error). On **PROD** use only **NON-DESTRUCTIVE** interactions (tab / FY / expand / filter / dialog-open-then-cancel; **NEVER** create/edit/delete real data); destructive functional flows belong to the pre-merge E2E suite. Pairs with rules 24 (render), 25 (persistence), 26 (cross-page), 31 (plausibility); where each runs is per `.claude/rules/testing-strategy.md`. A "verified" claim that only checked render/console/layout is **incomplete** — say so explicitly rather than implying functionality was tested.

## Independent Test Verification (a blind second tester checks the test)

33. **Any test verdict MUST be re-checked by a separate, context-blind agent (MANDATORY)**: whenever an agent OR a process performs **testing** (headed/headless Playwright, E2E, UI verification, a data-entry/persistence run, or anything emitting a pass/fail verdict), that verdict MUST be independently re-checked by a **SEPARATE agent that has NO context of how the first test ran** — given the **same inputs + the raw evidence** (screenshots/ARIA/console/DOM/persisted data), NOT the first agent's conclusions presented as fact — whose job is to judge **(a) was the testing actually done properly + completely (coverage — substance, rule 32) and (b) is the verdict correct (does the evidence support it — plausibility, rule 31)**. The verifier MUST be **adversarial** (find what was missed/wrong, not rubber-stamp); the first verdict MUST NOT be accepted/reported as done while the verifier **dissents** (T0 reconciles first). Applies to **T0's own test runs too** — T0 is never the sole verifier of a test it ran. This is the testing-specific, **auto-applied + context-isolated** form of the `operating-model.md` edge — it does not replace rules 26/29 or the supervisor gate, it names *who* verifies (a blind agent) and *what* it judges. Surface any skip verbatim ("independent test verification skipped because <reason>") — no silent skip. Full rule: `.claude/rules/independent-test-verification.md`.
