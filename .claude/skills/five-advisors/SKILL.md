---
name: five-advisors
description: "Run any question, idea, or decision through Five Advisors who independently analyze it, peer-review each other anonymously, and synthesize a final verdict. Based on Karpathy's LLM Council methodology. MANDATORY TRIGGERS: 'five advisors', 'run the advisors', 'convene the advisors', 'war room this', 'pressure-test this', 'stress-test this', 'debate this'. STRONG TRIGGERS (use when combined with a real decision or tradeoff): 'should I X or Y', 'which option', 'what would you do', 'is this the right move', 'validate this', 'get multiple perspectives', 'I can't decide', 'I'm torn between'. Do NOT trigger on simple yes/no questions, factual lookups, or casual 'should I' without a meaningful tradeoff (e.g. 'should I use markdown' is not an advisors question). DO trigger when the user presents a genuine decision with stakes, multiple options, and context that suggests they want it pressure-tested from multiple angles."
type: reference
...
---
## when to convene the advisors
Five Advisors is for questions where being wrong is expensive.
...
Bad advisor questions:
- "What's the capital of France?" (one right answer, no need for perspectives)
- "Write me a tweet" (creation task, not a decision)
- "Summarize this article" (processing task, not judgment)
Five Advisors shines when there's genuine uncertainty and the cost of a bad call is high. If you already know the answer and just want validation, the advisors will likely tell you things you don't want to hear. That's the point.
---
## the five advisors
...
When the user says "five advisors" (or any trigger phrase), do two things before framing:
**A. Scan the workspace for context.** The user's question is often just the tip of the iceberg. Their Claude setup likely contains files that would dramatically improve the advisors' output. Before framing, quickly scan for and read any relevant context files:
...
- Recent advisor transcripts in this folder (to avoid re-running the same ground)
...
If the question is too vague ("five advisors: my business"), ask one clarifying question. Just one. Then proceed.
Save the framed question for the transcript.
### step 2: convene the advisors (5 sub-agents in parallel)
...
```
You are [Advisor Name] on a panel of Five Advisors.
Your thinking style: [advisor description from above]
A user has brought this question to the advisors:
---
[framed question]
---
...
```
You are reviewing the outputs of the Five Advisors. Five advisors independently answered this question:
---
[framed question]
---
...
The chairman's job is to produce the final verdict. It follows this structure:
**ADVISORS' VERDICT**
1. **Where the advisors agree** — the points that multiple advisors converged on independently. These are high-confidence signals.
2. **Where the advisors clash** — the genuine disagreements. Don't smooth these over. Present both sides and explain why reasonable advisors disagree.
3. **Blind spots the advisors caught** — things that only emerged through the peer review round. Things individual advisors missed that other advisors flagged.
...
```
You are the Chairman of the Five Advisors. Your job is to synthesize the work of 5 advisors and their peer reviews into a final verdict.
The question brought to the advisors:
---
[framed question]
---
ADVISOR RESPONSES:
...
## Blind Spots the Advisors Caught
...
Be direct. Don't hedge. The whole point of Five Advisors is to give the user clarity they couldn't get from a single perspective.
```
### step 5: generate the report
After the chairman synthesis is complete, generate a visual HTML report and save it to the user's workspace.
**File:** `advisors-report-[timestamp].html`
...
6. **A footer** showing the timestamp and what was advised on
...
Save the complete transcript as `advisors-transcript-[timestamp].md` in the same location. This includes:
- The original question
- The framed question
- All 5 advisor responses
- All 5 peer reviews (with anonymization mapping revealed)
- The chairman's full synthesis
This transcript is the artifact. If the user wants to run the advisors again on the same question after making changes, having the previous transcript lets them (or a future agent) see how the thinking evolved.
---
## output format
Every session produces two files:
```
advisors-report-[timestamp].html    # visual report for scanning
advisors-transcript-[timestamp].md  # full transcript for reference
```
...
---
## example: advising on a product decision
**User:** "Five advisors: I'm thinking of building a $297 course on Claude Code for beginners. My audience is mostly non-technical solopreneurs. Is this the right move?"
...
**Chairman's Verdict:**
*Where the advisors agree:* The beginner solopreneur angle has real demand, but the current framing (Claude Code course) is too tool-specific and won't resonate with non-technical buyers.
*Where the advisors clash:* Price. The Contrarian says $297 is too high given competition. The Expansionist says it's too low for the value. The resolution likely depends on how much support and community access is bundled.
...
- **Don't convene the advisors for trivial questions.** If the user asks something with one right answer, just answer it. Five Advisors is for genuine uncertainty where multiple perspectives add value.
