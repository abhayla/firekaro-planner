...
> **This skill lives in the repo (`.claude/skills/`) on purpose.** That is the only location
> discoverable by **every** environment that has the repo — local Claude Code, **Claude Cowork**, and
> fresh clones. A copy in `~/.claude/skills/` is machine-LOCAL and is **invisible to Cowork** (a
> separate environment), so the repo is the home. It is fully portable — no project paths, no hardcoded
> numbers; it reads everything from the `WATI_*` env vars above (which Cowork must supply via its own
> environment secrets — it can't see this machine's `~/.config/wati/.env` either).
