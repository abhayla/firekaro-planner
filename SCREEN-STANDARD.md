...
> **Status:** v1.6 (all header-bearing screens conformed to `LeafPageHeader`; the table/form sub-standard (§10b) is the only remaining open item). Still a living document — see §0.
...
|---|---|---|---|
| **v1.6** | 2026-08-27 | **One-frame honesty + the unreachable state + basket disclosure + frame-tagged documents** (ADR-0006 Phase 2; SSOT `docs/adr/0006-real-frame-target-basket-inflation.md`, which outranks this prose). Rules established: (a) **one real return on screen** — every display surface reads the kernel's `derive().realBlendedReturn` (nominal deflated at GENERAL CPI); no screen may deflate at the household spending basket, and the identity is pinned numerically across all four seed personas rather than by convention (`FireMilestonesCard.binding.spec.ts`, gh #180 — the card and the hero had carried two different real returns, CI-green, because nothing compared the surfaces); (b) **name the rate, never the bare word "inflation"** — a screen showing a rate says whether it is your *spending basket* (what the target grows at) or *general inflation* (only the display deflator), and reads the live value from the kernel, never a hard-coded constant; (c) **an unreachable plan gets a first-class HEADLINE state**, not just a tile — gated on the solver's own signals (`requiredMonthlyReal` non-finite or `paceFireAge` null, both under `solved`), printing NO figure, warning-toned never red, and pointing at BOTH the moves (levers/slider) and the assumptions (deep-link to `/preferences#pref-section-inflation`); (d) **a defaulted assumption that moves the headline must be disclosed AND settable where it is explained** — Preferences carries the why (9% healthcare vs the 13–14% claims trend; education at 0% weight in a perpetual basket; disjoint 74/8/0/18 weights) beside the fields, plus the new savings step-up input; (e) **a persisted document carries the modelling frame it was captured under** — across a frame change the UI makes NO verdict, says so in plain words and offers a re-lock for a user-chosen document (plan baseline), or silently re-baselines an internal marker (lifecycle digest); it never fabricates a delta and never silently replaces a user's choice. | fire-goals/dashboard (✅ hero + Coast card + projection chart + plan-variance card), quick (✅ result chart + explainer copy), preferences (✅ disclosure + step-up field), glossary (✅ `spending-basket`, `savings-step-up`). |
| **v1.5** | 2026-08-27 | **Plan-lever picker pattern** (`LeverPicker.vue`, QN-5 / T-379, built in-session; mockup SSOT `docs/design/2026-08-27-quick-number-gap-hero/option-c-merged.html` plan card): a card of opt-in MOVES, each a checkbox + one-line note + its effect ALONE ("−₹X/mo · less to find", from a full re-solve through `derive()`), the switched-on set stacked and RE-SOLVED (never summed) for a plan-summary line, then the verbatim honesty line. Rules established: (a) nothing is assumed by default — every move is off until the user switches it on; (b) an inapplicable move is greyed WITH its reason ("your loan costs more than investing earns — prepay it"), never hidden and never faked; (c) the SAME component renders on `/quick` and inside the dashboard `AccelerationCard` (embedded, chrome-less), driven by ONE session-only `ui.whatIfLevers`, so a move switched on in one place is on in the other — the hero re-solves on the same set (`useFireDerive().activePlan`); (d) when the target is out of reach with AND without a move, the row shows how much of the corpus GAP it closes instead of a fake ₹/month; (e) "Make this my plan" writes only the persistable moves (step-up, target age, direct-plan return) through the existing stores and says which ones stay what-ifs. The existing ranked years-saved bars + bridge guards on the AccelerationCard are RETAINED; only the fixed-amount save-more slider went. | quick (✅), fire-goals/dashboard (✅ AccelerationCard body). |
