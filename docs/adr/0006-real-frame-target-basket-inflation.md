...
> **STATUS: ACCEPTED — Option E, approved by Abhay 2026-08-27 14:16 IST ("go with your recommendation").**
>
> **Decision.** (1) ONE frame: the projection runs in NOMINAL rupees end-to-end (expenses grow at the household basket, corpus at nominal returns, contributions at CPI + the real step-up) and is deflated at general CPI ONLY for display; the headline, the solver, the individual-FIRE path, the Monte Carlo band and the chart all read the same frame. (2) Re-grounded inputs: `healthcareInflation` 14% → **9%** (CPI-Health ≈ 4–7% + a 3–4 pp private-tariff/retiree-mix excess; the 13–14% Aon claims-cost trend belongs to the insurance-PREMIUM line, which already auto-flows into expenses); education leaves the perpetual retirement basket (weight 0; the 9% rate stays on the finite family-layer goals — and **Phase 1c** makes that finite-ness real: each dated goal inflates at its OWN bucket rate only until its due year and is then held FLAT in nominal rupees, because a bill paid in year 8 does not keep getting more expensive for the next twenty; a goal falling beyond the horizon inflates throughout); `inflationWeights` 60/20/10/10 → **74/8/0/18** (disjoint urban-household shares) ⇒ household basket ≈ **6.24%**, real target drift g ≈ 0.23%/yr. (3) `householdSavingsStepUpPercent` default 0 → **2 (% real/yr, tapering to 0 at age 50)** — the matched correction of the zero-real-wage-growth pessimism; on hydrate a stored value of exactly 0 (the old default) is treated as unset and takes the new default (a deliberate 0 is re-settable in Preferences; disclosed in the release note). (4) A first-class "unreachable at these assumptions" state on the hero — the kernel is never again bent to print a number. (5) The Coast/Barista card (#180) and every display surface read the kernel's ONE real return. (6) The bridge keeps flat today's-₹ expenses AND zero return on the drawn pool as a STATED pair of offsetting simplifications (net direction ambiguous; revisit only together). (7) `healthcareCorpusReservationPercent` (0.2) is audited for the triple-count in the implementation and reduced if confirmed. **RESOLVED in Phase 1c: no triple-count, and the reservation is NOT reduced — it DRIFTS at `healthcareInflation` (9%), not at the household basket.** The three healthcare rupees are distinct: the auto-flowed premium is an insurance cost inside expenses; the basket's 8%-weighted healthcare bucket is recurring healthcare SPEND inside the ongoing-expenses corpus; the reservation is a buffer against medical SHOCKS, whose price rises at medical inflation. Sizing it as 20% of the base today while letting it grow toward ~44% of the base by year 30 is not an error — it IS the "healthcare weight of a household rises with age" mechanism, and it is bounded by the buffer's own price path because the legs are explicit schedules rather than one blended rate. Cost, stated: +0.4 to +1.0 years of FIRE age across the four seeds (the conservative direction).
>
> **Guards.** Frame + inputs + step-up land in ONE commit (a frame-only landing ships a defeatist headline — FinTech HIGH-6). The regression lock (§6) is behavioural: negative control (raising a bucket rate must raise needReal/needNominal and never lower FIRE age), positive control (all buckets = CPI ⇒ the model is INTERNALLY CONSISTENT at g = 0 — a real target that does not drift, asserted with an epsilon, not `==`; NOT byte-identical to the pre-ADR headline, which also moves by the two conservative kernel corrections in the Phase-1 report §3), two-frame agreement (headline vs chart crossover ± 1 yr), hero/Coast real-return identity across all personas, and the honesty ratchet SCOPED to the frame-only leg. Expected per-persona movement (FinTech table): frame-only Sharmas 57 / Mehtas 51 / Iyers 58 / Mauryas 69; with the step-up 54 / 51 / ~55 / ~68 — every deviation from that table is explained per persona in the PR. **UNITS (Phase 1d).** Every persona figure in this ADR is the DISPLAYED age — `householdFireAge = anchorAge + ceil(yearsToRegular)`, the number on screen — never the raw fractional `anchorAge + yearsToRegular`. As landed through Phase 1c the displayed ages move **56 -> 56 (Sharmas), 51 -> 51 (Mehtas), 57 -> 58 (Iyers), 68 -> 69 (Mauryas)**, with raw years-to-FIRE 25.42 / 6.00 / 19.83 / 24.92 against the pre-ADR 25.667 / 5.333 / 18.917 / 23.500. Quoting a raw post-value (55.42) against a displayed pre-value (56) reads as an improvement that did not happen; `headline-plausibility.spec.ts` now asserts the <= 70 bound on BOTH figures so the two can never drift apart again.
>
> **Superseded:** the #20 "collapse both sides to CPI" decision in `derive.ts`. **Related:** gh #167 (closes), gh #180 (closes), ADR-0004 (step-up segments). The FinTech validation summary and the original Option-D draft follow, kept for the record.
...
- **Status:** Accepted (Option E) — 2026-08-27
- **Date:** 2026-08-27
...
**The positive control (corrected, Phase 1b).** When a household's four inflation buckets are all equal to general CPI — `inflation = healthcareInflation = educationInflation = housingInflation` (e.g. all 6%) — `blendedInflation` returns exactly `π` (`fire-math.ts`: a weighted mean of identical values), so `g = (1+π)/(1+π) − 1 = 0`. The real target is then constant and `calculateYearsToTarget` walks a fixed-target loop. **The model is INTERNALLY CONSISTENT at g = 0** — that is what the control asserts.

It is **NOT byte-identical to the pre-ADR-0006 headline**, and the earlier wording claiming it was is wrong. Two conservative kernel corrections land alongside the frame change and move the headline for every household regardless of `g` (Phase-1 report §3): (1) monthly compounding is now the true equivalent `(1+r)^(1/12) − 1` rather than `r/12`, which was silently over-compounding every projection; (2) the target is resolved at a continuous fractional year rather than at `floor(months/12)`, so eleven months of corpus growth no longer race a target frozen in January. Both push every persona LATER. The control is therefore stated as `g == 0` plus a constant real target, with an epsilon — never as equality against a pre-ADR figure.
