...
>
> **Guards.** Frame + inputs + step-up land in ONE commit (a frame-only landing ships a defeatist headline — FinTech HIGH-6). The regression lock (§6) is behavioural: negative control (raising a bucket rate must raise needReal/needNominal and never lower FIRE age), positive control (all buckets = CPI ⇒ byte-identical headline, asserted with an epsilon, not `==`), two-frame agreement (headline vs chart crossover ± 1 yr), hero/Coast real-return identity across all personas, and the honesty ratchet SCOPED to the frame-only leg. Expected per-persona movement (FinTech table): frame-only Sharmas 57 / Mehtas 51 / Iyers 58 / Mauryas 69; with the step-up 54 / 51 / ~55 / ~68 — every deviation from that table is explained per persona in the PR.
>
