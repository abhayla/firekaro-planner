...
- **It's a RECIPIENT-side, adaptive cap** — it tracks *the user*, across *all* businesses, not just us.
  Our test number `<owner-test-number — see GLOBAL.md>` had received broker-marketing broadcasts; its marketing read-rate /
...
- This is the most reliable way to reach an engaged user — and how we'll verify delivery to
  `<owner-test-number — see GLOBAL.md>` (have the user message the business number, then send a session message).
