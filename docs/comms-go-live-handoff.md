...
- **B1. Supabase RLS — ⚠️ ESCALATED 2026-06-10 (Supabase is now emailing weekly CRITICAL advisor warnings).** The Security Advisor flags 2 CRITICAL on project `zymbhuwuguzeueslwhyz`: `rls_disabled_in_public` ("table publicly accessible") + `sensitive_columns_exposed` ("sensitive data publicly accessible via the API"). **Exposure:** Supabase auto-exposes a PostgREST Data API (`https://zymbhuwuguzeueslwhyz.supabase.co/rest/v1/`) keyed by the anon key; with RLS off, anyone with that anon key can read/write/delete EVERY table (PAN/salary/family/financial) — no second layer. Latent CRITICAL (the anon key isn't published by us, so not a confirmed active leak — but the safety layer is removed). **THE FIX IS SAFE — the old "enabling breaks the app" note was WRONG for this app** (verified 2026-06-10): the app connects as the **`postgres` service role** (bypasses RLS) and uses **no `@supabase/supabase-js`/anon-key path** (frontend→Hono→Prisma→pooler only). So:
  - **Option A (simplest):** disable the Data API (Supabase → Settings → API) — app never uses it → zero impact, whole surface removed.
  - **Option B (recommended posture):** enable RLS deny-all on all 24 tables (`ALTER TABLE … ENABLE ROW LEVEL SECURITY`) — service-role Prisma bypasses it, anon REST denied. Claude can run this with the `postgres` `DATABASE_URL`.
  **Gated (prod DB/infra change) — Abhay's go.** Recommend doing it (don't defer to 5W) given the weekly CRITICAL warnings + the finance-PII surface.
