...
- **B11. DEPLOY today's wave to firekaro.com — APPROVED by Abhay 2026-08-27 19:03 IST ("We will deploy it"); executes in the next session, end to end, no further approval needed.** On `main` and NOT live: the Quick Number front door (#166 #168 #173 #175 #178 #179) and ADR-0006 (#181: honest inflation frame, re-grounded basket, 2% real step-up, unreachable state, #180). Order is not optional: `cd server && npm run prisma:migrate:deploy` (migration `20260827120000_adr0006_assumptions_columns`; an unmigrated DB makes Preferences saves 500) → app restart → Tier-1 smoke → Tier-1.5 UI check (login renders; dashboard hero shows the "today's rupees, at age N" frame note; Preferences save does not 500). Runbook `docs/DEPLOY.md` §3/§8. Existing users see "re-lock to compare" on the plan slot (never a fabricated delta); the off-track WhatsApp nudge is suppressed for FY 2026-27. Say go and I run it over SSH (~15 min) and do the post-deploy checks. **Status 2026-09-13 (session-start audit): STILL NOT EXECUTED** — prod `/api/health` OK on the pre-wave build; `main` at `e1f183a` is only rules/docs commits past the approved wave, so the approved bundle is unchanged; local gates green (type-check ×2, lint, 100 FE spec files + 187 BE tests). Per the global one-deploy-window rule (~21:00–23:30 IST) it needs a fresh "go" in-window; PR #177 (T-379 QN-5 levers, CI green, no conflicts) could ride the same window if verified first (R5: name it in the pre-deploy brief).
- ~~**B10. DECIDE the inflation model for gh #167**~~ — RESOLVED 2026-08-27 14:16 IST: Option E approved (ADR-0006 Accepted). Original text: DECIDE the inflation model for gh #167 (Tier-0 honesty, moves EVERY user's headline) — 2026-08-27 14:10 IST.** `docs/adr/0006-real-frame-target-basket-inflation.DRAFT.md` carries the research + the FinTech verdict. The fix as written in #167 makes FIRE ages 71/69/93 for the seed personas (it is the #20 model in a new frame); the real root cause is the 4-bucket basket itself (non-disjoint buckets; 14% healthcare is a claims-cost trend, not a price rate). Options on the table: **E** (recommended by FinTech and by me): honest nominal frame + re-grounded basket (healthcare 9%, education out, weights 74/8/0/18 → ~6.24%) + 2% real savings step-up default + an 'unreachable' state — every persona moves ≤ 1 year; **F** (time-boxed): one rate, 6% everywhere, basket removed from the kernel + display until re-grounded; **D** (the Opus draft's damped-healthcare curve — rejected by FinTech as an uncited curve rescuing an uncited rate). Nothing is built until you pick.
- ~~**B9. Rotate the Supabase database password**~~ — DECLINED by Abhay 2026-08-27 19:03 IST ("not going to rotate"); closed, no action. Original: Rotate the Supabase database password (low urgency, 2026-08-27 10:12 IST).** While pulling `DATABASE_URL` from the VPS into a local gitignored `server/.env` (for rule-25 server-mode verification), a shell-pattern slip echoed the full connection string into the Claude session transcript on this PC (not git, not the fleet bus, not any remote). Per `security-baseline.md` a secret that surfaced anywhere gets rotated: Supabase → Settings → Database → reset password, then update `/var/www/firekaro/server/.env` on the VPS + `pm2 reload firekaro-api`, and the local `server/.env`. Nothing else holds it.
