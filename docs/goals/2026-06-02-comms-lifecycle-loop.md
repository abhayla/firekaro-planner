...
are unit-tested. The fail-closed allowlist (`WATI_TEST_RECIPIENTS`, only `<owner-test-number — see GLOBAL.md>`) + the
...
(`decideSend`) + fail-closed allowlist. Until Abhay flips A6, sends reach ONLY `<owner-test-number — see GLOBAL.md>`. The whole
...
- **Verified (rules 24/25):** set a number via the Preferences UI → confirm persisted via
  `GET /api/comms/consent` → a test send to that number (= `<owner-test-number — see GLOBAL.md>`) reaches `DELIVERED` via the
  existing Wati path.
...
- **Verified end-to-end** to `<owner-test-number — see GLOBAL.md>`: trigger the endpoint against a seeded household → the right
...
- The fail-closed allowlist is the test harness — verify real `DELIVERED` to `<owner-test-number — see GLOBAL.md>` only.
