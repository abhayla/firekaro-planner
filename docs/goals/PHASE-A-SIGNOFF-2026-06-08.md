...
### #59 — tax.ts mutation closure (A7.6) — ⚠️ SUBSTANCE MET; literal ≥85% gated by equivalents (commits 2339aef + metadata disables)
- **68.85% → 83.74%** (+14.9pts; 273 killed / 51 survived / 326). 75 exact-value differential tests (`tax-mutation-kill.spec.ts`) lock slab / surcharge-bands + marginal-relief + cap / rebate + rebate-marginal-relief / age-exemption / NPS-cap / regime-recommendation / FY-coverage at every boundary; plus comment-only `Stryker disable` on proven-equivalent config-metadata (financialYear/assessmentYear/isDefault — never read by the engine).
...
- **Literal ≥85% NOT reached (83.74%)** — the last ~1.3pts are equivalent mutants no test can kill (redundant conditions like the `else if … && taxableIncome > rebateLimit` whose `>` is always true in that branch; the redundant `age != null` guard; contiguous-inclusive-band equality ops). Pushing further risks metric-gaming (a metadata-disable pass already incidentally removed 2 incidental kills), which rule 31 forbids on honesty-critical code. Surfaced explicitly per "no silent skips". The honesty-critical math is protected; the number is gated by equivalence.
