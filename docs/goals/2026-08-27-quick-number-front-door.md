...
**Reviewed:** context-blind review 2026-08-27 (6/10 NOT READY → 13 fixes applied, re-verified — see PROJECT-LOG D-2026-08-27-01). **Pinned version for dispatch = the commit that carries this line** (workers cite it).
**Execution:** via `/get-work-done` (one contract per stage below; each stage = one PR, CI-gated, independent checker). Stage order is a dependency order — QN-3 and QN-2 are kernel/hero work that QN-1 consumes. **QN-2 OWNS the `ui.quick` type + store field (declared there, read there); QN-1 only writes it** — so QN-2's PR type-checks on its own.
...
**Pre-made decisions:**
- New pure module `src/lib/required-contribution.ts`: `requiredMonthlyContributionFor({ snapshot, assumptions, lens, targetAge })` → `{ requiredMonthlyReal, currentMonthlyReal, gapReal, needReal, haveAtTargetReal, paceFireAge, needNominal, swrUsed }`. Implementation = **binary search on the household real monthly contribution** through the existing `derive()` path (NOT a parallel formula) so step-up, bridge/accessibility, horizon-SWR, family layer and lens are all honoured. Bounds: `lo = 0`, `hi = max(10 × current real monthly contribution, 5 × household monthly take-home, ₹5,00,000)`; tolerance ₹100/month; cap 60 iterations. If `derive()` at `hi` still does not reach the target age → `requiredMonthlyReal = Infinity` (the UI renders "beyond any realistic monthly amount — move the age"); `paceFireAge = null` when the current pace never reaches the number by `planToAge`. Never NaN (rule 31). **Monotonicity is a precondition of binary search and MUST be locked by a property test** (`kernel-invariants.property.spec.ts`): for every seed + fast-check perturbation, `derive()`'s years-to-FIRE / target-age reachability is non-increasing in the real monthly contribution across `[lo, hi]` — bridge, horizon-SWR, healthcare reservation and the NPS post-tax offset included. If the test finds a non-monotone region, STOP: file an issue and fall back to a monotone-guaranteed scan (step ₹1,000, then bisect within the first reachable step) — do not ship a silently-wrong solver.
...
- `FireHero.vue` (Option-D hero, D-2026-06-10-09) gains: **one headline = target age** (replace "You'll FIRE at age N" with "To retire at {target} you'll need ₹X (today's money · ₹Y in {year})"; the current-pace age moves to the demoted annotation exactly as in the mockup), the four tiles (need / have-by-target / gap / **do this**), the retirement-age slider (40–70), the "+3 years → ₹" hint, and the gut-feel line when `ui.quick.guess` is set. **QN-2 declares** `ui.quick?: { guess?: number; completedAt?: string; createdIds?: string[]; directPlans?: boolean }` in `src/stores/ui.ts` (persisted inside the existing `ui` document blob — no new column) and reads it; QN-1 later writes it. **Server carve-out (the ONE permitted `server/` edit, verified 2026-08-27):** `server/src/routes/planner.ts` `uiBodySchema` is a strip-mode `z.object` — an unknown `quick` key is silently dropped on PUT. QN-2 adds `quick: quickPrefsSchema.optional()` (guess number ≥0, completedAt ISO string, createdIds string[], directPlans boolean — all optional) to `uiBodySchema` and to the merge in the same route, plus a `planner.integration.spec.ts` case (PUT with `quick` → GET returns it; PUT without `quick` leaves an existing `quick` intact — the merge-not-replace guarantee). Server gate: `cd server && npm run type-check && npm run lint && npm run test:unit`. No Prisma change (prefs is `Json`). Confidence band + since-away delta + plan-variance KPI + bridge subline are RETAINED (non-removable honesty surfaces).
- **New** `GapTone = "short" | "surplus" | "unknown"` + `resolveGapTone(gapReal)` in `dashboard-verdict.ts` (amber / green / neutral; NaN or ±Infinity → `unknown`, makes NO claim). The existing plan-variance `HeroTone`/`resolveHeroTone` and its rule-31 no-baseline contract are **untouched** — two signals, two types, both shown.
...
**Pre-made decisions:**
- Route `/quick` (**no `meta.layout`**, exactly like `Splash`/`Login` — `App.vue` only checks `=== 'sidebar'`; do not invent a new layout key), page `src/pages/QuickNumber.vue`, components under `src/components/quick/` (`QuickCard.vue`, `LakhInput.vue`, `QuickResult.vue`). Entry: Splash "Start my own plan" → `/quick` (the 7-step wizard becomes "Refine your plan", reachable from the result CTA and the sidebar; completing `/quick` sets the EXISTING `household.wizardCompleted = true` — the flag means "setup complete", and the quick path IS the setup; the path taken is recorded in `ui.quick.completedAt`. NO new flag, NO schema change). Demo "Try the sample" unchanged; every demo-only affordance stays behind `isServerMode()`.
- **Exactly the 10 cards of Option C**, same order, same copy (copy lives in `src/lib/quick-number-copy.ts` so it is testable and one-place): gut-feel · you (age + retire-at) · spend (+ take-home, with the sanity line) · ALL investments (copy names stocks/ETFs/gold/crypto/bonds/plots/second flat; only the home excluded; + the Direct/Regular/Not-sure chip row → `ui.quick.directPlans`) · monthly investing (names stock buys + both PFs) · spouse investments (toggle) · kids (count + age) · kids' big costs (education, post-grad, weddings) · big purchase (toggle + live delta) · home loan (toggle; EMI, rate, years left). Money in lakh with live ₹ preview; "So far…" strip after card 3.
- **Persistence = real household data, never a side store** (5W principle 2 + the storage invariant): mapping in `src/lib/quick-number.ts` `applyQuickAnswers(household, answers)` → adult member(s) with ages + `targetRetirementAge`; `expenses.avgMonthly`; one salary line for take-home; ONE investment line per person, type `"MutualFunds"` (exact `investmentTypeSchema` enum value; equity-classified by `investment-traits.ts`), label "All investments (quick estimate)", tagged `subtypeData: { quick: true }` (existing Json column — `Investment.source` already exists with a DIFFERENT meaning, 'Direct'/owner-entity id, and MUST NOT be reused); kids as DEPENDENT members with `educationStage` from age; plannedFuture lines: education (kind `education`, targetYear = kids reach 18), post-grad (education, +4), weddings (`marriage`, kids reach **30** — the advisor's figure in the transcript, "25 years from now, at the age of 30"), big purchase (`general`, targetYear = now+6 unless asked); home loan as a Liability (auto-flows the EMI — so spend must be entered **without** EMI when a loan is present: card 3 copy adapts: "…excluding the home-loan EMI, we add that from the next card" — resolve the double-count explicitly); gut-feel guess + quick metadata in the **`ui` document** (`ui.quick = { guess, completedAt, createdIds }` inside `userUiPrefs.prefs` — schemaless JSON, merged server-side, so NO Prisma migration; `HouseholdConfig` columns are explicit and `server/` is off-limits). Idempotent: re-running quick updates the lines listed in `ui.quick.createdIds` (fallback: the `subtypeData.quick` tag), never duplicates.
...
**Pre-made decisions:**
- Extend `src/lib/lever-catalog.ts` (not a new module) with: `step-up-10` (sets `householdSavingsStepUpPercent` 10 via ADR-0004 — already supported by the kernel), `delay-3` (target+3), `direct-plans` (available only when `ui.quick.directPlans !== true` — QN-1's investments card carries a 3-chip row "Are your mutual funds direct plans? Direct / Regular / Not sure" that writes `ui.quick.directPlans` (true only for Direct); absent/unsure → the lever stays available with the note "if you're already on direct plans, ignore this"; a What-If lever value: +0.008 on equity-class returns applied through the existing scenario/lever-value path — `user_assumptions` columns are explicit, so NO new persisted assumption field; "Make this my plan" maps it to the existing `equityReturn` override (+0.8 pp) with a note in the preferences hint. A per-holding regular/direct flag is a separate good-to-have issue), `no-prepay-roll-emi` (available only when a liability exists with rate < expected equity return; effect = a `ContributionSchedule` segment adding the EMI from the loan's end-year — ADR-0004 segments, no kernel change). Existing `trim-expenses` and `save-more` stay.
...
- The lever card (Option C) lives in `src/components/quick/LeverPicker.vue` and is added to the dashboard INSIDE `AccelerationCard.vue` as its body. **`useAcceleration.ts`, `lever-impact.ts`, `lever-bands.ts`, the `bridgeBinding`/`baselineReachable` guards and `AccelerationCard.binding.spec.ts` are RETAINED** (bug-#22 / rule-31 history): the "biggest win" KPI keeps its years-saved ranking; LeverPicker adds the ₹/month "less to find" view driven by `required-contribution.ts` (which itself goes through `derive()`, so it is bridge-aware). The binding spec is EXTENDED with the picker, never deleted. Only the fixed-extra-amount "save more" presentation is replaced.
...
- [ ] QN-1: Splash → `/quick` → 10 cards → result → dashboard, persisted as real household data (+ `ui.quick` metadata), idempotent, EMI not double-counted, ZERO Prisma changes; the `uiBodySchema` carve-out landed with its merge-not-replace integration test; every "total" question copy passes the ALL/stocks/one-exclusion spec.
...
- `src/`, `e2e/`, `docs/`, `SCREEN-STANDARD.md`, `CLAUDE.md` (one line) only — plus the SINGLE carve-out in QN-2: `server/src/routes/planner.ts` `uiBodySchema` + its integration spec (nothing else under `server/`). Otherwise never `server/` — verified 2026-08-27: every new datum this contract needs fits an EXISTING column (`plannedFuture.kind`, `Investment.subtypeData` Json, `Investment.contributionSchedule` Json, `userUiPrefs.prefs` Json, `Member.targetRetirementAge`). If a stage finds otherwise, STOP and file an issue, never `.claude/`, never `5Wealths\`.
