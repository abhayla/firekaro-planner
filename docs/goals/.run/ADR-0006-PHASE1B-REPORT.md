...
| MEDIUM-4 | `fix(monte-carlo)` | `derive().bandContributionSchedule` — the real inflow with the within-year CPI step applied (factor 0.969067 at 6% CPI — the mean of `(1.06)^(-k/12)` for k = 1..12; the 0.96766 first written here was arithmetically wrong, corrected in Phase 1d). Wired into `useFireDerive`, `lifecycle-digest`, and both "mirrors production EXACTLY" specs. |
