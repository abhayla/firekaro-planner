...
`917972672473`. The autonomous run **MUST NOT trigger real sends** — it proves the watchdog with mock
...
1-min heartbeat for a throwaway project, stop pinging, and confirm one real alert arrives to 917972672473 —
...
- **No real sends in the autonomous run** — mock adapters only; live-delivery is Abhay-gated (spend + outward, `decision-authority.md`). The fail-closed WhatsApp allowlist (`917972672473`) is NEVER widened by this run.
