...
Per Abhay's standing instruction, **all testing sends go ONLY to his number `<owner-test-number — see GLOBAL.md>`; no other
...
- `WATI_TEST_RECIPIENTS` (comma-separated, in `server/.env`) — the allowlist. Dev value: `<owner-test-number — see GLOBAL.md>`.
...
Regression-locked by `wati-client.spec.ts` (blocks a stranger's number, allows `<owner-test-number — see GLOBAL.md>`,
...
First live test to `<owner-test-number — see GLOBAL.md>` returned 200 but the message **FAILED**:
