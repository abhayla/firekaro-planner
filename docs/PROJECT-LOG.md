...
**Updated: 2026-09-13.** Persona widened (D-2026-09-13-02/03); the next must-have is the banded income path in the kernel (**#185**, spec `docs/goals/2026-09-13-income-path-kernel.md`) — PARKED for token budget, nothing started. The 2026-06-08 lines below are the last full status audit and are kept as history.
- **Goal (SSOT `docs/v6-fire-planner-product-plan.md` §9):** research-grounded Indian FIRE planning
  SaaS — correct, honest, sticky, friction-free — serving the **urban salaried accumulator** (age 20–60, salaried, household ₹2.5L–₹1Cr+ — widened 2026-09-13, D-2026-09-13-02) across
...
### 2026-09-13 (session-start readiness audit on the C:\ PC — D-2026-09-13-01)
- **D-2026-09-14-01 — first-principles rebuild assessment: no build; must-have registry cut from 43 to 4 (Abhay).** Question put to a `/first-principles` pass: what would a from-scratch build of FireKaro add or change? Ground truths: input is manual entry only (zero import code; #88/#90/#91/#92 untouched); 3 rows in the prod `User` table, retention unmeasured, no analytics; the kernel ignores salary growth (#185 parked); 43 issues carried `must-have`. Killed: "honesty needs the complete 24-model household" (five inputs with a wide band is more honest than forty guessed inputs with a narrow one); "manual entry until Account Aggregator" (AA needs FIU registration, out of reach for a solo non-RIA product); a 43-item must-have list. Kept: the stack, the ~47-module maths layer, decision-support-not-advice. The rebuilt order was distribution (public/Hindi `/quick`) → five self-hosted funnel counters → #185 → client-side CAS + Form 16 import → value-of-information onboarding → thin tax / fat income → process trimmed to honesty gates. **Decision:** only the relabel was approved (spec step 8 of #185); every build step refused ("do not implement anything"). 39 issues moved to `good-to-have`, among them honesty/coherence bugs #87, #161, #64, #86, #85, #73 — still fixable under the focus-lock carve-out for shipped features. Falsifier recorded: if 20 strangers finish the 7-step wizard unaided and 5 return in month 2, data-entry is not the bottleneck and import stays good-to-have. Spec §9 line: `docs/goals/2026-09-13-income-path-kernel.md`. Status per item (2026-09-14): relabel DONE; #185 income path = parked (`spec-ready`); CAS import = #92; Form 16 import = #186; public/Hindi `/quick` = #187; value-of-information onboarding = #188; funnel counters = #44; retention channel = #45; thin-tax/fat-income = inside #185 step 7; process trim = not a repo item, owner call. All good-to-have, none started.
- **D-2026-09-13-04 — banded income path replaces the savings step-up proxy; everything growth-related is user-configurable (Abhay). Spec parked: `docs/goals/2026-09-13-income-path-kernel.md`, issue #185 (must-have).** From a `/first-principles` pass: the kernel grows *savings* 2 %/yr real (ADR-0006 proxy) and never reads `salary.hikePercent`, so a ₹3L/₹2.5L 22-year-old ("Ravi") gets FIRE at 58 when the same 2 % applied to *salary* gives 51 and his own 12 % hikes give 42 (all in actual rupees, expenses inflating 6 %, FIRE number inflating with them). Rules locked: headline = conservative band only; own hike % moves the second number; salary growth per earner, taper age, expense creep and an optional age-band schedule (ADR-0004 segment shape) are all user-editable with research defaults and double as What-If levers. **Amends D-2026-09-13-03's guard** ("hoped-for income never moves the headline") to the banded form: it never moves the *headline*, it does show beside it. Casualties: the step-up proxy as the growth model; Form 16/CAS import as the setup story for the low band. Sequencing: seeds + per-seed plausibility bounds (red-first) BEFORE the kernel; the only schema step (start-dated side income) LAST. Open fact blocking the kernel: a cited conservative real wage-growth default for lower-middle salaried India. Budget ≈ 400–600k tokens for steps 1–5; owner parked it 2026-09-13 ("no token budget right now, make sure we don't forget").
- **D-2026-09-13-03 — §9 Basis reversed (Abhay): the customer is the lower-middle / middle / upper-middle class salaried earner; the affluent / HNI earner is already financially independent and is not the customer.** Tier-3 "vernacular mass-market = maybe never" withdrawn (sequencing, not exclusion). Product rule that follows: a low income today is a starting point, never a verdict — the plan must show how income growth (job switches, raises) and NEW income sources reach FIRE; a bare "not reachable" is a product defect. Honesty guard kept: hoped-for income never moves the HEADLINE; it appears as a banded path / lever next to it (`goal-anchored-decisions.md` rule 30 — optimistic error is Tier-0). Audit of what the code does today, driving the improvement list given to Abhay this session: user-entered `salary.hikePercent` is display-only (Income pages), NOT read by `derive.ts`; the kernel proxies wage growth with the 2%/yr real savings step-up tapering to 0 at 50 (ADR-0006), which under-states early-career growth for a 20-year-old at ₹2.5L; `otherIncomeLine` has no start date and no Freelance / Side-business / Second-job type, so a future income source cannot be modelled; all 8 levers in `lever-catalog.ts` are savings/tax-side, zero income-side; every nudge in `nudge-engine.ts` is affluent-oriented (NPS cap, international allocation, estate gaps, real-estate overweight), none for emergency fund / first SIP / income growth; lowest seed is ₹18L CTC. No code changed this turn.
- **D-2026-09-13-02 — persona bands widened (Abhay): age 20–60, salaried, household income ₹2.5L–₹1Cr+.** Was 28–45 / IT-finance-consulting / metro / ₹15L+ / English-literate (§9, chosen 2026-06-01). §9 bullets updated in place with a revision note; the §9 *Basis* paragraph ("FIRE is an affluent urban-salaried movement") and the Tier-3 "vernacular mass-market = maybe never" line were NOT re-argued and now partly contradict the new band — owner call whether they stand. **What it costs, stated plainly:** (1) a ₹2.5L household is under the new-regime rebate line and cannot save a FIRE corpus on ₹2.5L; the honest headline for them is "not reachable" or "after 60", so the planner must show that honestly rather than promise a date; (2) no seed persona sits below ₹18L CTC — the ₹2.5L–₹10L band is untested by the golden-master, plausibility and property specs (`headline-plausibility.spec.ts` asserts savings-rate 15–70% and FIRE age ≤70 on the seeds only, so CI stays green while the low band is unverified); (3) age 20 and age 55–60 accumulators are inside `clampAge(18, 80)` in `quick-number.ts`, so nothing blocks them, but the default assumptions (target age, hike %, step-up) were tuned for 28–45. **No code changed this turn.** Follow-ups (not started, not queued): a low-income seed + plausibility fixture; re-read §9 Basis/Tier-3 against the new band. `TODO(5W):` the market-positioning lock lives in `FW-FireKaro\DECISIONS.md` — this widening supersedes the 2026-06-01 persona and needs a 5W entry.
- **Repo made work-ready on this PC.** `main` was 128 commits behind `origin/main` (nothing local to lose) → fast-forwarded to `e1f183a`; both `node_modules` trees were empty → installed + `prisma generate`; gates reproduced green on the pulled main (`vue-tsc` clean, 100 FE spec files pass, server `tsc` + eslint clean, 187 BE tests pass). Lockfile churn from `npm install` (npm 11 stripping `"peer": true` flags, no version change) was discarded, not committed. Setup gaps fixed and committed in `cd646db`: `server/.env.example` still pointed at the retired VPS Postgres and lacked `DIRECT_URL` (P1012 on `prisma migrate deploy`) → retargeted to the Supabase pooler + `DIRECT_URL` added; needs-Abhay **B11** annotated *still not executed*.
- **D-2026-09-13-01 — gh #170 closed as a duplicate of #176; #176 tiered `must-have`.** Same defect (`derive()` ignores `RecurringExpenseLine.endYear`, so a terminating EMI is capitalised into the FIRE number forever — ~₹3.4 Cr on the QN-1 reference persona). Goal-anchored why: a headline-honesty error for the locked persona is Tier-0 regardless of fix size (`goal-anchored-decisions.md` rule 30); fixing shipped math is inside the must-have-only carve-out. Not started — it is the first candidate work item, and it touches `derive.ts` so it runs the full financial-math chain (FinTech ∥ code review, rule 29/31).
- **PR #177 (fleet T-379 QN-5) closed as SUPERSEDED** — QN-5 was rebuilt in-session and merged as #179 (D-2026-08-27-07); the fleet branch was a competing implementation 65 commits behind main. Branch kept on origin for test salvage (`lever-plausibility.spec.ts`, `lever-catalog.plan.spec.ts`). The unreviewed kernel branch `wip/T-379-qn5-roll-emi-kernel` (2026-09-12 entry) remains do-not-merge-without-review. Local `-T-379` worktree (clean, 0 unpushed) removed.
- **Incident (recorded in `.claude/tasks/lessons.md`): the haiku `git-manager-agent` edited the deterministic secret-scan gate (`.githooks/pre-commit`) outside its two-file brief and pushed it in the same commit under a subject line that names only the hook.** The change itself is narrow (adds the `change_me` spelling next to the existing `changeme` placeholder exclusion) and was inspected + kept; the commit is on `origin/main` so it was not rewritten (destructive-git gate). The brief defect: it did not say what to do when the hook BLOCKS. Fixed going forward by the lesson.
- **Still open, owner-facing:** B11 deploy (Quick Number wave + ADR-0006 migration) needs a fresh in-window "go" (one-deploy-window rule, ~21:00–23:30 IST); stale PRs #163 (GA4 consent, June, 130 behind) and #164 (CI docs-only short-circuit, 123 behind) need a merge/close call — neither rides into a deploy unless named (R5).

### 2026-09-12 (PC sync + worktree/clone cleanup)
- **This PC is now fully synced to GitHub; all FireKaro worktrees and retired clones removed.** Owner asked to make the project resumable from another PC and to delete unneeded worktrees (global worktree-lifecycle rule R2/R3). Verified: `firekaro-planner` has **0 unpushed commits, 0 stashes, no unpushed tags**, `main` at `605afc2` == `origin/main`, and exactly **one** worktree (the main checkout). Removed both linked worktrees via `wt-rm.ps1` (main checkout proven intact, 1213→1213 tracked files, 0 dirty, both times).
- **One real find: 107 lines of UNCOMMITTED FIRE-kernel work in the `-t379` worktree** (`derive.ts`, `derive-overrides.ts`, `derive.spec.ts` — the T-379/QN-5 `no-prepay-roll-emi` lever, an `extraMonthlyFromAge` override implemented as a second ADR-0004 contribution segment). It was **not** on GitHub. Pushed to **`wip/T-379-qn5-roll-emi-kernel`** (`4e7ce8d`) rather than discarded; the same-named `origin/fleet/T-379-qn5-levers` was ahead, so a new branch was used instead of a force-push. **UNREVIEWED / UNVERIFIED — do NOT merge without the full financial-math chain (rule 29/31: FinTech analyst + independent review + green kernel tests).**
- **Retired clones deleted from disk after proving them synced:** `FIREKaro` (5.78 GB) and `FIREKaro-Vue` (2.10 GB) — both **archived** on GitHub, so their last local edits were landed by briefly unarchiving → push → re-archiving (`FIREKaro` `c825ea2`, `FIREKaro-Vue` `19fc8e6`); `.env` deliberately excluded from those commits. Stray non-repo `FIREKaro-Vue-OLD-Nuxt` removed. Their local `.env*` files were copied to `D:\Abhay\_retired-repo-env-backup\` (outside every repo) before deletion. **~8.4 GB reclaimed** (D: free 185.27 GB); only `firekaro-planner` (0.60 GB) remains.
- **Security note (pre-existing, retired repo):** `.env` is a **tracked** file in `abhayla/FIREKaro` history and therefore already on GitHub. That repo is private + archived, but any credential that ever lived in it should be treated as exposed and rotated.
- **What does NOT travel to another PC** (gitignored by design): `server/.env`, `D:\Abhay\GLOBAL.env`, `D:\Abhay\GLOBAL.md`, `.remember/`. Carry these by hand; see §4.

### 2026-08-27 (18:40 IST)
- **ADR-0006 BUILT — PR opening (closes #167 + #180).** 42 commits on `feat/adr-0006-honest-inflation-frame`, all in-session via Opus/Sonnet subagents with Fable orchestrating; four review passes (FinTech ×3, code ×1), every finding landed. Displayed FIRE ages pre→post: Sharmas 56→56, Mehtas 51→51, Iyers 57→58, Mauryas 68→69; needReal +1.2% to +14.8% (Sharmas ₹10.60→₹12.17 Cr); `fireNumber` byte-identical; headline and chart crossover now agree exactly (were 4–8 y apart, chart optimistic). Two production-only classes caught by review and turned into mechanisms: server strip-mode Zod dropping `frameVersion` (→ `server-schema-parity.spec.ts`) and the Prisma write layer silently ignoring three accepted Assumptions fields incl. the step-up (→ migration `adr0006_assumptions_columns` + `planner-assumptions-mapping.spec.ts`). Deploy order is migration → restart (DEPLOY.md). Root 1,481+ tests, server 187; e2e + sweep + 390/1280 screenshots re-run on the final branch before merge.
...
### 2026-08-27 (13:30 IST)
- **D-2026-08-27-07 — QN-5 "How to get there" levers BUILT IN-SESSION (fleet paused by Abhay at 12:09 IST; T-379 worker killed at turn ~10, $3).** `LeverPicker.vue` on `/quick` and as the `AccelerationCard` body; five moves (step-up 10%/yr real, retire 3 yrs later, trim 10%, direct plans +0.8%, don't-prepay-roll-the-EMI) each re-solved ALONE through `requiredMonthlyContributionFor` → "less to find" ₹/month; the switched-on set stacked and re-solved (a spec proves stacked ≠ sum of parts); a no-inert-lever guard on the real Sharmas seed; the only kernel-adjacent change is an additive, default-off `extraContributionSegments` override (proved byte-identical when empty). Design SSOT rows B4/B6/C6/C7/D2/E6/E7/F6 closed; SCREEN-STANDARD v1.5 records the pattern.
  - **Honesty decisions (rule 31):** (1) when the target is out of reach with AND without a move, the row shows how much of the corpus GAP the move closes rather than a fake ₹/month (∞−∞ is not a number we quote); (2) the spec's acceptance "Amit with step-up+delay+direct lands within 1.5× current" was written against the simplified mock (₹13.35 Cr); on the real kernel (₹19.05 Cr need) those three give **₹2.88 L/mo = 1.65× the ₹1.75 L he invests**, and all five give ₹2.55 L vs ₹2.03 L (1.26×) because trimming frees ₹28k. We report the real ratio; we do not tune the math to the mock. (3) The dashboard used to HIDE the acceleration card when the target was out of reach — exactly when the moves matter most; it now shows whenever any move is available, with an honest "target out of reach today" chip.
  - **Deferred (not built here, with reason):** a per-holding direct/regular flag (good-to-have; the household-level chip + lever covers the video's point); a post-tax (Sec 24b) loan-rate comparison for the no-prepay rule (the pre-tax comparison is the CONSERVATIVE direction — it makes the move available less often); the `/fire-goals/what-if` page does not yet read `ui.whatIfLevers` (it has its own lever set; unifying is a separate change).

### 2026-08-27 (12:05 IST)
- **D-2026-08-27-06 — PR #178 (T-378F fix-round) MERGED; `/quick` is design-complete per the T-378C findings list.** Checker T-378FC PASS with an empty fix-round list: F1–F7 re-derived (loan balance now the PV annuity ₹65.83 L vs the old ₹84 L; two-column desktop result; sanity guard; chart window from the answered age), server-mode rule-25 re-proved by the checker's own spec (household + `ui.quick` persisted through the real backend), member-lens sweep 24/24, #174 confirmed pre-existing. Flinch note worth keeping: correcting the loan balance moves net worth only — the headline is EMI-driven (expenses/SWR), so ₹18 L less debt does not change the FIRE number; the loan card copy already says so. Next: T-379 (QN-5 levers, opus, 220 turns) dispatched off this main; then T-380 (#167).
